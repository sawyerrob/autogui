...
def ocr():
    im = ImageGrab.grab(bbox=(940,511,1120,537))
...
def check_original():
    ...
    pyautogui.click(252, 400, button="left",interval=2)
    # 鼠标移动查询按钮，并且点击查询
...
def down_original(path1):
    ...
    pyautogui.click(560, 400, button="left",interval=2)
    # 鼠标移动点击下载按钮

    pyautogui.click(995, 598, button="left",interval=2)
    # 鼠标移动选择自定义

    pyautogui.click(800, 640, button="left",interval=2)
    # 鼠标移动选择正卷下载
    ...
    pyautogui.click(1234, 385, button="left", interval=2)
    # 鼠标移动点击关闭下载窗口
def down_copy(path2):
...
for case_number in range(0, len(case_number_list)):
    ...
    pyautogui.click(321, 305, button="left", interval=2)
                    # 鼠标移动到输入案号的界面,并单击左键
    ...
    err= check_original()
                ...
                # 查询是否存在正本
    mmkpath = cwd + "\\" + "(2017)"+cases
    mmkpath1 = cwd + "\\" + "(2017)"+ cases + "\\" + "正卷"
    mmkpath2 = cwd + "\\" +  "(2017)"+cases + "\\" + "副卷"
    if operator.eq(err, "查i旬 的数据不啼宇茌"):
        clean("nodown_list",mmkpath)
        time.sleep(5)
        continue
    else:
